 ...
 */
contract MON is ERC20, Ownable, ReentrancyGuard {
    // Token消费事件
    ...
    constructor(
        ...
        require(fundPoolAddress != address(0), "MON: Fund pool address cannot be zero");
        // 设置资金池合约地址
    ...
    function reward(address to, uint256 amount) external nonReentrant {
        require(to != address(0), "MON: Cannot reward to zero address");
        require(amount > 0, "MON: Reward amount must be greater than 0");
    ...
    function consume(address user, uint256 amount, bytes32 reason, string calldata description) external nonReentrant {
        require(user != address(0), "MON: User address cannot be zero");
        require(amount > 0, "MON: Consumption amount must be greater than 0");
        require(balanceOf(user) >= amount, "MON: Insufficient balance");
    ...
    function mintToFundsPool(uint256 amount) external onlyOwner nonReentrant {
        require(amount > 0, "MON: Mint amount must be greater than 0");
    ...
    function transferFromFundsPool(address to, uint256 amount) external onlyOwner nonReentrant {
        require(to != address(0), "MON: Cannot transfer to zero address");
        require(amount > 0, "MON: Transfer amount must be greater than 0");
