<template>
  <section class="transaction-flow">
    <div class="section-header">
      ...
      <span id="transaction-count">{{ transactions.length }}笔交易</span>
    </div>
...
const getTransactionDescription = (transaction) => {
  if (transaction.type === 'sell') {
    return `${transaction.agentName || transaction.fromAgent} 出售了 ${transaction.amount} ${transaction.tokenSymbol}`;
  } else if (transaction.type === 'buy') {
    return `${transaction.agentName || transaction.fromAgent} 购买了 ${transaction.amount} ${transaction.tokenSymbol}`;
  }
  return '';
};
</script>
